...
const nodes = [
    ...
    "={{ $('Pick Backlog').item.json.contact_url }}", [420, 60]),
  {
    ...
    parameters: {
      jsCode: [
        ...
        '    // Confidence travels with the verdict so the team can see at a',
        '    // glance whether an address came from the company\\u0027s own domain',
        '    // or from somewhere less certain.',
        '    email_valid: best ? ("found:" + best.source + ":" + best.confidence) : "no_email_found",',
  ...
  },
  {
    ...
    // A DNS hiccup must not lose a batch of extractions.
    onError: 'continueRegularOutput',
    alwaysOutputData: true,
  },
  {
...
const connections = {
  ...
  'Extract Contacts': { main: [[{ node: 'Verify MX', type: 'main', index: 0 }]] },
  'Verify MX': { main: [[{ node: 'Shape Update', type: 'main', index: 0 }]] },
